...
public class ThymeController {
    ...
    public String viewHomePage(Model model) {
        List<ChatVO> chatViews = getChats().stream()
                .map(chat ->
                        new ChatVO(chat.id(), chat.name(),
                                ...
                                String.format(LAST_POST_MSG, "Kevin Callanan", "Hey!"),
    ...
    public String viewChatPage(Model model) {
        List<MessageGroupVO> messageGroups = getMessageGroups(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID());
        model.addAttribute("messageGroups", messageGroups);
        return "chatView";
    }

    private List<Chat> getChats(){
        return List.of(new Chat(UUID.randomUUID(), "test Chat 1", new ArrayList<>(), LocalDateTime.now()),
    ...
    private List<MessageGroupVO> getMessageGroups(UUID loggedInUserId, UUID author1, UUID author2){
        String author1thumbnail = "https://images.unsplash.com/photo-1550745165-9bc0b252726f?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2940&q=80"; // TODO - default value like anon would be good
        String author2thumbnail = "https://images.unsplash.com/photo-1567532939604-b6b5b0db2604?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=3087&q=80"; // TODO - default value like anon would be good
        String author3thumbnail = "https://images.unsplash.com/photo-1531123897727-8f129e1688ce?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=3087&q=80"; // TODO - default value like anon would be good

        List<MessageGroupVO> messageGroups = new ArrayList<>();
        String thumbnailUrl = "https://images.unsplash.com/photo-1550745165-9bc0b252726f?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2940&q=80"; // TODO - default value like anon would be good
        UUID authorId = null;
        ...
        for (Message msg : getMessages(loggedInUserId, author1, author2)){
            if(!msg.author().equals(authorId)){
                ...
                thumbnailUrl = (authorId.equals(loggedInUserId)) ? author1thumbnail
                        : (authorId.equals(author1)) ? author2thumbnail
                        : author3thumbnail;
            }
    ...
    private List<Message> getMessages(UUID loggedInUser, UUID author1, UUID author2){
        ...
        return List.of(
                ...
                new Message(UUID.randomUUID(), "I'm good! How's tricks?", chatId, LocalDateTime.now(), loggedInUser),
                new Message(UUID.randomUUID(), "Gerrup, ye daisy!", chatId, LocalDateTime.now(), author2),
                new Message(UUID.randomUUID(), "I wouldn't event mind but haven't chatted in yonks.", chatId, LocalDateTime.now(), author2),
